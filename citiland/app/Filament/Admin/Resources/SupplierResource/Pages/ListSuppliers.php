    ...
    public static function cetakPerformanceAnalysis()
{
    $data = \DB::table('suppliers')
        ->select(
            ...
            \DB::raw('(SELECT COUNT(DISTINCT KodeRetur) FROM returs WHERE returs.kode_supplier = suppliers.kode_supplier) as return_count'),
            \DB::raw('(SELECT COALESCE(SUM(JumlahBahanBaku), 0) FROM returs WHERE returs.kode_supplier = suppliers.kode_supplier) as total_returns'),
            \DB::raw('(SELECT COALESCE(SUM(HargaRetur * JumlahBahanBaku), 0) FROM returs WHERE returs.kode_supplier = suppliers.kode_supplier) as total_return_value')
        )
        ->get()
