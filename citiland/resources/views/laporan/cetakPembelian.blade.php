...
<body>
    ...
    <footer>
        ...
        <p>Total Transaksi: {{ $data->count() }}</p>
        <p>Total Nilai Pembelian: Rp {{ number_format($data->sum('HargaBahanBaku'), 0, ',', '.') }}</p>
        <p>Total Jumlah Pembelian: {{ $data->sum('JumlahPembelian') }}</p>
        <p>Rata-rata Nilai Pembelian: Rp {{ number_format($data->avg('HargaBahanBaku'), 0, ',', '.') }}</p>
    </footer>
</body>
</html>